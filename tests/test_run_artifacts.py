...
from batho.storage.engine import BathoDatabase, SCHEMA_VERSION
...
def test_register_artifact_removed_from_storage():
    """from batho.context.storage import register_artifact raises ImportError."""
    with pytest.raises(ImportError):
        from batho.context.storage import register_artifact  # noqa: F401


def test_persist_json_removed():
    """from batho.context.storage import persist_json raises ImportError."""
    with pytest.raises(ImportError):
        from batho.context.storage import persist_json  # noqa: F401
...
def test_registry_check_module_deleted():
    """from batho.integrity.checks.registry import RegistryIntegrityCheck raises ImportError."""
    with pytest.raises(ImportError):
        from batho.integrity.checks.registry import RegistryIntegrityCheck  # noqa: F401
