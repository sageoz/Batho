...
def test_git_diff_entries_to_file_changes_added_modified_deleted(
    ...
    assert any(
        c.path == "a.py" and c.change_type == FileChangeType.ADDED for c in changes
    )
    assert any(
        c.path == "b.py" and c.change_type == FileChangeType.DELETED for c in changes
    )


def test_files_from_diff_parses_formats_and_skips_unsafe(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    root = tmp_path / "repo"
    root.mkdir()
...
def test_detect_file_changes_handles_missing_snapshot_and_detects_add_mod_del(
    ...
    assert any(
        c.path == "mod.py" and c.change_type == FileChangeType.MODIFIED for c in changes
    )
    assert any(
        c.path == "gone.py" and c.change_type == FileChangeType.DELETED for c in changes
    )


def test_auto_detect_changes_detects_add_modify_delete_and_filters(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    root = tmp_path / "repo"
    root.mkdir()
    ...
    assert any(
        c.path == "deleted.py" and c.change_type == FileChangeType.DELETED
        for c in changes
    )
    assert any(
        c.path == "mod.py" and c.change_type == FileChangeType.MODIFIED for c in changes
    ...
    assert any(
        c.path == "new.py" and c.change_type == FileChangeType.ADDED for c in changes
    )
    assert not any(c.path == "bin.py" for c in changes)
    assert not any(c.path == "large.py" for c in changes)


def test_reindex_files_covers_skip_paths_and_parser_flow(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    root = tmp_path / "repo"
    root.mkdir()
    ...
    monkeypatch.setattr(
        batho, "is_ignored", lambda fp, *_a, **_k: fp.name == "ignored.py"
    )
    ...
    monkeypatch.setattr(
        batho.default_detector,
        "get_extractor",
        lambda fp, _content: _Extractor() if fp.name == "good.py" else None,
    )
    monkeypatch.setattr(
        batho,
        "registry_get_extractor",
        lambda ext: _Extractor() if ext == ".py" else None,
    )
    ...
    monkeypatch.setattr(
        batho,
        "_strip_files",
        lambda *_a, **_k: stripped.__setitem__("count", stripped["count"] + 1),
    )
...
def test_metrics_and_hash_helpers_cover_edge_paths(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    root = tmp_path / "repo"
    root.mkdir()
    ...
    monkeypatch.setattr(batho, "load_ignore_spec", lambda _r: None)
    monkeypatch.setattr(
        batho, "is_ignored", lambda fp, *_a, **_k: fp.name == "ignored.txt"
    )
...
def test_needs_backfill_load_graph_and_strip_files_helpers(tmp_path: Path) -> None:
    ...
    assert (
        _needs_metrics_backfill({"indexes": {"idx": {"stats": "bad", "metrics": {}}}})
        is True
    )
    assert (
        _needs_metrics_backfill({"indexes": {"idx": {"stats": {}, "metrics": {}}}})
        is True
    )
    assert (
        _needs_metrics_backfill(
    ...
    _strip_files(
        graph,
        ["", "src/a.py", str((tmp_path / "src" / "b.py").resolve())],
        root=tmp_path,
    )
...
def test_cmd_query_branches(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(
        batho, "_load_index_metadata", lambda _c: {"current_index_id": "idx1"}
    )
    monkeypatch.setattr(
        batho, "get_config_cached", lambda: {"bsg": {"query": {"default_limit": 10}}}
    )
    ...
    monkeypatch.setattr(
        batho, "_load_index_metadata", lambda _c: {"current_index_id": ""}
    )
    assert cmd_query(a) == 1


def test_cmd_bsg_modes_and_errors(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    ...
    writes = []
    monkeypatch.setattr(
        batho, "_write_json", lambda path, data: writes.append((path.name, data))
    )
    ...
    assert (
        cmd_bsg(argparse.Namespace(root=str(root), mode="compressed", budget=200)) == 0
    )
    ...
    assert (
        cmd_bsg(argparse.Namespace(root=str(root), mode="hierarchical", budget=200))
        == 0
    )
...
def test_backfill_index_metrics_updates_and_noop(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    ctn = tmp_path / ".ctn"
    ctn.mkdir()
    ...
    monkeypatch.setattr(
        batho, "get_config_cached", lambda: {"indexer": {"max_file_size_kb": 64}}
    )
    monkeypatch.setattr(
        batho,
        "_collect_repo_metrics",
    ...
    monkeypatch.setattr(
        ...
        lambda *_a, **_k: saved.__setitem__("called", saved["called"] + 1),
    )
...
def test_cmd_index_early_failure_paths(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    # Invalid root path.
    assert (
        cmd_index(argparse.Namespace(root=str(tmp_path / "missing"), log_json=False))
        == 1
    )
    ...
    monkeypatch.setattr(
        ...
        lambda *_a, **_k: [SimpleNamespace(status="M", path="a.py")],
    )
    monkeypatch.setattr(
        batho,
        "_git_diff_entries_to_file_changes",
        lambda *_a, **_k: [SimpleNamespace(path="a.py")],
    )
    monkeypatch.setattr(
        batho,
        "incremental_patch",
        lambda *_a, **_k: {"success": False, "error": "nope"},
    )
    assert cmd_index(args) == 1
...
def test_patch_subcommands_index_and_snapshot_paths(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    root = tmp_path / "repo"
    root.mkdir()
    ...
    monkeypatch.setattr(
        batho,
        "record_failure_rule",
        lambda *_a, **_k: {"entry_id": "e1", "dont_rule": "rule"},
    )
    ...
    monkeypatch.setattr(
        batho,
        "load_snapshot",
        lambda *_a, **_k: {"bsg": {"quality": {"warnings": ["w1"]}}},
    )
    monkeypatch.setattr(
        batho, "_extract_bsg_quality_warnings", lambda *_a, **_k: ["w1"]
    )
    monkeypatch.setattr(batho, "_emit_bsg_quality_warnings", lambda *_a, **_k: None)
    assert _cmd_patch_snapshot_based(args_snap, root, ctn) == 0


def test_patch_management_commands(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch, capsys
) -> None:
    root = tmp_path / "repo"
    root.mkdir()
    ...
    monkeypatch.setattr(
        "batho.time_machine.list_patch_operations", lambda *_a, **_k: [patch]
    )
    assert (
        cmd_patches(
            argparse.Namespace(
    ...
    assert (
        cmd_patches(
            argparse.Namespace(
                root=str(root), operation_type=None, base_snapshot=None, format="json"
            )
        )
        == 0
    )
    ...
    monkeypatch.setattr(
        "batho.time_machine.load_patch_operation", lambda *_a, **_k: None
    )
    assert (
        cmd_patch_info(
            argparse.Namespace(root=str(root), patch_id="missing", format="json")
        )
        == 1
    )

    monkeypatch.setattr(
        "batho.time_machine.load_patch_operation", lambda *_a, **_k: patch
    )
    assert (
        cmd_patch_info(
            argparse.Namespace(root=str(root), patch_id="op1", format="summary")
        )
        == 0
    )
    assert (
        cmd_patch_info(
            argparse.Namespace(root=str(root), patch_id="op1", format="json")
        )
        == 0
    )

    monkeypatch.setattr(
        "batho.time_machine.get_patches_for_snapshot", lambda *_a, **_k: []
    )
    assert (
        cmd_patch_chain(
            argparse.Namespace(root=str(root), snapshot_id="s2", full=False)
        )
        == 1
    )

    monkeypatch.setattr(
        "batho.time_machine.get_patches_for_snapshot", lambda *_a, **_k: [patch]
    )
    assert (
        cmd_patch_chain(
            argparse.Namespace(root=str(root), snapshot_id="s2", full=False)
    ...
    assert (
        cmd_patch_chain(argparse.Namespace(root=str(root), snapshot_id="s2", full=True))
        == 0
    )
    ...
    assert (
        cmd_apply_patch(
            argparse.Namespace(
                root=str(root),
                diff_file=str(root / "missing.diff"),
                patch_id=None,
                base_snapshot="s1",
                dry_run=False,
            )
    ...
    monkeypatch.setattr(
        "batho.time_machine.parse_unified_diff",
        lambda _d: [
            SimpleNamespace(path="a.py", change_type=SimpleNamespace(value="modified"))
        ],
    )
    assert (
        cmd_apply_patch(
    ...
    monkeypatch.setattr(
        batho,
        "incremental_patch",
        lambda *_a, **_k: {"success": True, "new_snapshot_id": "s3"},
    )
    assert (
        cmd_apply_patch(
            argparse.Namespace(
    ...
    monkeypatch.setattr(
        batho, "incremental_patch", lambda *_a, **_k: {"success": False, "error": "bad"}
    )
    assert (
        cmd_apply_patch(
            argparse.Namespace(
    ...
    monkeypatch.setattr(
        Path,
        "read_text",
        lambda *_a, **_k: (_ for _ in ()).throw(RuntimeError("read err")),
    )
    assert (
        cmd_apply_patch(
            argparse.Namespace(
                root=str(root),
                diff_file=str(bad_file),
                patch_id=None,
                base_snapshot="s1",
                dry_run=False,
            )
    ...
    monkeypatch.setattr(
        "batho.time_machine.apply_deltas_to_snapshot", lambda *_a, **_k: "s4"
    )
    assert (
        cmd_apply_patch(
            argparse.Namespace(
    ...
    monkeypatch.setattr(
        "batho.time_machine.apply_deltas_to_snapshot", lambda *_a, **_k: None
    )
    assert (
        cmd_apply_patch(
            argparse.Namespace(
    ...
    monkeypatch.setattr(
        batho,
        "record_failure_rule",
        lambda *_a, **_k: {"entry_id": "e3", "dont_rule": "rule3"},
    )
    assert (
        cmd_apply_patch(
            argparse.Namespace(
    ...
    assert (
        cmd_cherry_pick(
            argparse.Namespace(
                root=str(root), patch_id="missing", target_snapshot="s9", dry_run=False
            )
        )
        == 1
    )
    ...
    assert (
        cmd_cherry_pick(
            argparse.Namespace(
                root=str(root), patch_id="op1", target_snapshot="s9", dry_run=True
            )
        )
        == 0
    )
    ...
    monkeypatch.setattr(
        "batho.time_machine.apply_deltas_to_snapshot", lambda *_a, **_k: "s10"
    )
    assert (
        cmd_cherry_pick(
            argparse.Namespace(
                root=str(root), patch_id="op1", target_snapshot="s9", dry_run=False
            )
        )
        == 0
    )
    ...
    monkeypatch.setattr(
        batho,
        "record_failure_rule",
        lambda *_a, **_k: {"entry_id": "e2", "dont_rule": "rule2"},
    )
    assert (
        cmd_cherry_pick(
            argparse.Namespace(
