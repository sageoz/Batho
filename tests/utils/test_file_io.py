...
def test_interprocess_locking(tmp_path: Path):
    ...
    # After releasing lock1, lock2 should be acquirable again
    with lock2:
        pass


def test_is_locked_by_other_no_file_creation(tmp_path: Path):
    ...
    assert InterProcessLock.is_locked_by_other(lock_file) is False
    assert not lock_file.exists()
