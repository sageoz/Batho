"""File outline handler — Nested code structure for navigation.

Builds a nested outline for files based on BSG data and line-range containment.
"""
...
LOGGER = get_logger(__name__, component="bridge_core.handlers.outline")
...
def handle_file_outline(deps: WorkspaceDeps, params: dict) -> dict:
    """Handle GET /api/v2/file/outline

    Returns nested outline for a file based on BSG data.

    Args:
        deps: Workspace dependencies
        params: Query parameters (required: file)

    Returns:
        dict with keys: file, nodes, node_count
    """
    ...
    try:
        if not deps.bsg_manager:
            return {
                "ok": False,
                "error": "BSG data not available",
                "data": {"file": file_path, "nodes": [], "node_count": 0},
            }
        ...
        LOGGER.error("file_outline_error", error=str(e), file=file_path)
        return {
            "ok": False,
            "error": str(e),
            "data": {"file": file_path, "nodes": [], "node_count": 0},
