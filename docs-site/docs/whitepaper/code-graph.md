...
Batho v1.3.1 introduces `ArrowGraph`, a columnar, memory-mapped graph backend that serves as a drop-in alternative to `InMemoryGraph` for large codebases. It uses a three-phase lifecycle:
...
Batho v1.3.1 supports lossless, bidirectional graph-to-code reconstruction, allowing a developer or LLM agent to rebuild the exact source file from the graph.
