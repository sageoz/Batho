...
flowchart TD
    ...
    B --> C["sageoz/batho@v1.2.1 composite action"]
...
jobs:
  batho:
    uses: sageoz/batho/.github/workflows/batho-index.yml@v1.2.1
    with:
      root: "."
      python-version: "3.12"
      batho-ref: "pypi"
...
- **Always up to date**: Pin to a tag (`v1.2.1`) or follow `main` for latest features.
