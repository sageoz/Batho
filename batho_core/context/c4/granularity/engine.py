...
class GranularityDecisionEngine:
    ...
    def __init__(self):
        self.logger = get_logger(
            self.__class__.__name__, component="granularity_engine"
        )
    ...
    def decide_granularity(
        self, metrics: RepositoryMetrics, override: Optional[GranularityLevel] = None
    ) -> GranularityDecision:
        """
    ...
    def _decide_with_rules(self, metrics: RepositoryMetrics) -> GranularityDecision:
        ...
        if decision_score < 100:
            ...
            reasoning = (
                f"Small repository ({entity_count} entities) with low complexity"
            )
            confidence = 0.9
            ...
            reasoning = (
                f"Large repository ({entity_count} entities) needs high-level view"
            )
            confidence = 0.85
        else:
            level = GranularityLevel.ADAPTIVE
            reasoning = (
                f"Massive repository ({entity_count} entities) needs adaptive approach"
            )
            confidence = 0.75
    ...
    def _get_settings_for_level(
        self, level: GranularityLevel, metrics: RepositoryMetrics
    ) -> Dict[str, Any]:
        ...
        if level == GranularityLevel.FINE:
            ...
            if metrics.entity_count > 10000:
                settings.update({"include_components": False, "max_containers": 100})
    ...
    def get_memory_estimate(self, metrics: RepositoryMetrics) -> int:
        ...
        relationship_memory = (
            metrics.relationship_count * 0.05
        )  # 0.05MB per relationship
    ...
    def validate_decision(self, decision: GranularityDecision) -> bool:
        ...
        if decision.confidence < 0.5:
            self.logger.warning(
                "Low confidence in granularity decision", confidence=decision.confidence
            )

        # Check settings consistency
        settings = decision.settings

        if decision.level == GranularityLevel.COARSE and settings.get(
            "include_components"
        ):
            self.logger.error(
                "Inconsistent decision: coarse granularity with components"
            )
            return False
