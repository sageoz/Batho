...
If `batho` is found and prints a version, skip to Workflow 2.
...
3. **After Batho upgrade** — `pip install --user --upgrade batho` (or `uv tool upgrade batho` or `pipx upgrade batho`). Restart all MCP clients.
